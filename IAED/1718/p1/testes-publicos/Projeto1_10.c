...
void compress_matrix() {
    ...
    unsigned long list_index[MAX_OFFSET], list_offset[MAX_LINES];
    unsigned long i, j, k, lines_count, lines[MAX_LINES], control, counter,
    offset, larger_offset, position;
    long long l;
    if (dens > 0.5) {
        printf("dense matrix\n");
    } else {
        ...
        */
        for (i = 0; i < lines_count; i++) {
            list_lines[i].index = lines[i];
            list_lines[i].elements = 0;
            for (j = 0; j < nelem; j++) {
        ...
        /*
        for (i = 0; i < lines_count; i++) {
            printf("%lu %lu\n", list_lines[i].index, list_lines[i].elements);
        }
        */
        ...
        larger_offset = 0;
        memset(list_values, 0, sizeof(list_values)/sizeof(list_values[0]));
        memset(list_index, 0, sizeof(list_index)/sizeof(list_index[0]));
        memset(list_offset, 0, sizeof(list_offset)/sizeof(list_offset[0]));
        for (i = 0; i < lines_count; i++) {
            if (list_lines[i].elements > 0) {
                j = 0;
                while (j < list_lines[i].elements) {
                    for (k = 0; k < nelem; k++) {
                        if (elements[k].line == list_lines[i].index) {
                            temporary[j] = elements[k];
                            j++;
                        }
                    }
                }
                sort(temporary, 1, list_lines[i].elements);
                /*
                printf("%lu ", list_lines[i].index);
                for (j = 0; j < list_lines[i].elements; j++) {
                    printf("%f ", temporary[j].value);
                }
                printf("\n");
                */
                counter = 0; offset = 0;
                while (counter < list_lines[i].elements) {
                    for (j = 0; j < list_lines[i].elements; j++) {
                        position = temporary[j].column - minj;
                        if (list_values[position + offset] == 0) {
                            counter++;
                        } else if ((list_values[position + offset] != 0)) {
                            offset++;
                        }
                    }
                }
                for (j = 0; j < list_lines[i].elements; j++) {
                    ...
                    list_index[position + offset] = list_lines[i].index;
                }
                list_offset[list_lines[i].index - mini] = offset;
...
void refresh_info() {
     ...
     * isto e, mini, minj, maxi, maxj, nelem, size e dens;
     *
     * Argumentos: Nao tem.
     * Output: Nao tem.
     */
